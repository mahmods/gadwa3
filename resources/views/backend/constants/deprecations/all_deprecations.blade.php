...
@section('breadcrumb')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="#">{{trans('constants.deprecations')}}</a>
            </li>
...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group">
            <a class="btn green" href="{{route('getAddDeprecation')}}"><i class="fa fa-level-down"></i> {{trans('constants.add_deprecation')}}
            </a>
        </div>
        <br clear="all">
        <br clear="all">
        ...
        <div class="portlet box gadwa">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i>{{trans('constants.deprecations')}}</div>
                <div class="tools"></div>
            </div>
            <div class="portlet-body">
                <table class="table table-striped table-bordered table-hover" id="sample_2">
                    ...
                    <tr>
                        ...
                        <th>{{trans('constants.percentage')}}</th>
                        <th>{{trans('backend.action')}}</th>
                    </tr>
                    </thead>
                    <tbody>
                    @foreach($deprecations as $i=>$deprecation)
                        <tr class="odd gradeX">
                            <td>{{$i+1}}</td>
                            <td>{{$deprecation->title}}</td>
                            <td>{{$deprecation->percentage}}</td>
                            <td>
                                <a class="btn btn-outline btn-circle btn-sm purple"
                                   href="{{route('getDeprecationById',['deprecationId'=>$deprecation->id])}}">
                                ...
                                <form id="form_delete" class="form-delete-c" method="post"
                                      ...
                                      action="{{route('deleteDeprecationById',['deprecationId'=>$deprecation->id])}}">
                                    {{ csrf_field() }}
                                </form>
                            </td>
                        </tr>
