...
@section('breadcrumb')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="#">{{trans('expense.expenses')}}</a>
            </li>
        </ul>
...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group">
            <a class="btn green" href="{{route('getAddExpense')}}"><i class="fa fa-level-down"></i> {{trans('expense.add_expense')}}
            </a>
        </div>
        <br clear="all">
        <br clear="all">
        ...
        <div class="portlet box gadwa">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i>{{trans('expense.all_expenses')}}</div>
                <div class="tools"></div>
            </div>
            <div class="portlet-body">
                <table class="table table-striped table-bordered table-hover" id="sample_2">
                    ...
                    <tr>
                        <th>#</th>
                        <th>{{trans('expense.study')}}</th>
                        <th>{{trans('expense.title')}}</th>
                        <th>{{trans('expense.description')}}</th>
                        <th>{{trans('expense.value')}}</th>
                        <th>{{trans('expense.space')}}</th>
                        <th>{{trans('expense.count')}}</th>
                        ...
                        <tr class="odd gradeX">
                            ...
                            <td>
                                <a class="btn btn-outline btn-circle btn-sm purple"
                                   href="{{route('getExpenseById',['expenseId'=>$expense->id])}}">
                                ...
                                <form id="form_delete" class="form-delete-c" method="post"
                                      ...
                                      action="{{route('deleteExpenseById',['expenseId'=>$expense->id])}}">
                                    {{ csrf_field() }}
                                </form>
                            </td>
                        </tr>
