    ...
    <div class="col-md-12">
        <div class="portlet box gadwa">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-gift"></i>{{trans('competitor.update_competitor')}}</div>
                <div class="tools">
            ...
            <div class="portlet-body form">
                <!-- BEGIN FORM-->
                <form method="post" action="{{route('updateCompetitorById',['competitorId'=>$competitor->id])}}">
                    {{ csrf_field() }}
                    <div class="form-body">
                            ...
                            <select id="study_id" name="study_id" id="single" class="form-control select2">
                                ...
                                @foreach(\App\Repositories\StudyRepository::getAllStudiesView() as $study)
                                    <option value="{{$study->id}}"
                                            @if($competitor->study_id==$study->id) selected @endif>{{$study->title}}</option>
                                @endforeach
                        ...
                        <div class="form-group">
                            ...
                            <input name="title" value="{{$competitor->title}}" type="text" class="form-control"
                                   placeholder="{{trans('competitor.title')}}">
                            @if($errors->has('title'))
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="description" class="form-control"
                                      placeholder="{{trans('competitor.description')}}">{{$competitor->description}}</textarea>
