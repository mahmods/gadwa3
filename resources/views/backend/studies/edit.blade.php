    ...
    <div class="col-md-12">
        <div class="portlet box gadwa">
            ...
            <div class="portlet-body form">
                <!-- BEGIN FORM-->
                {!! Form::open(['class'=>'form-horizontal','name'=>'form','novalidate','method'=>'put','url'=>route('updateStudyById',['studyId'=>$study->id])]) !!}
                ...
                <div class="form-body">
                    <div class="form-group">
                        ...
                        <input name="title" value="{{$study->title}}" type="text" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <input name="phone" value="{{$study->phone}}" type="number" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <input name="space" value="{{$study->space}}" type="number" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <input name="email" value="{{$study->email}}" type="text" class="form-control"
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="location" class="form-control"
                                  placeholder="{{trans('studies.location')}}">{{$study->location}}</textarea>
                    ...
                    <div class="form-group">
                        ...
                        <textarea name="description" class="form-control"
                                  placeholder="{{trans('studies.description')}}">{{$study->description}}</textarea>
                        ...
                        <select name="user_id" id="single" class="form-control select2">
                            ...
                            @foreach(\App\Repositories\UserRepository::getAllUsersView() as $user)
                                <option value="{{$user->id}}"
                                        @if($user->id==$study->user_id) selected @endif>{{$user->name}}</option>
                            @endforeach
                        ...
                        <select name="sector_id" id="single" class="form-control select2">
                            ...
                            @foreach(\App\Repositories\SectorRepository::getAllSectorsView() as $sector)
                                <option value="{{$sector->id}}"
                                        @if($sector->id==$study->sector_id) selected @endif>{{$sector->title}}</option>
                            @endforeach
                        ...
                        <select name="currency_id" id="single" class="form-control select2">
                            ...
                            @foreach(\App\Repositories\CurrencyRepository::getAllCurrenciesView() as $currency)
                                <option value="{{$currency->id}}"
                                        @if($currency->id==$study->currency_id) selected @endif>{{$currency->code}}</option>
                            @endforeach
                        ...
                        <select id="country_id" name="country_id" id="single" class="form-control select2">
                            ...
                            @foreach(\App\Repositories\CountryRepository::getAllCountriesView() as $country)
                                <option value="{{$country->id}}"
                                        @if($country->id==$study->country_id) selected @endif>{{$country->title}}</option>
                            @endforeach
...
@section('js')
    <script>
        ...
        $.post({
            url: "<?php echo route('getCities')?>",
            data: {id: id, cityId: "<?php echo $study->city_id?>"},
            success: function (res) {
                //console.log(res);
                $("#city_id").html(res);
            }
