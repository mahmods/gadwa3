...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group">
            <a class="btn green" href="{{route('getAddContent')}}"><i class="fa fa-level-down"></i> {{trans('content.add_content')}}
            </a>
        </div>
        <br clear="all">
        <br clear="all">
        ...
        <div class="portlet box gadwa">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i>{{trans('content.all_contents')}}</div>
                <div class="tools"></div>
            </div>
            <div class="portlet-body">
                <table class="table table-striped table-bordered table-hover" id="sample_2">
                    ...
                    <tr>
                        <th>#</th>
                        <th>{{trans('content.content_type')}}</th>
                        <th>{{trans('content.title')}}</th>
                        <th>{{trans('content.description')}}</th>
                        <th>{{trans('content.hint_step')}}</th>
                        <th>{{trans('content.hint_order')}}</th>
                    ...
                    @foreach($contents as $i=>$content)
                        <tr class="odd gradeX">
                            <td>{{$i+1}}</td>
                            <td>{{trans('content.'.$content->content_type)}}</td>
                            <td>{{$content->title}}</td>
                            <td>{{$content->description}}</td>
                            <td>{{$content->hint_step}}</td>
                            <td>{{trans('content.'.$content->hint_step)}}</td>
                            <td>{{$content->hint_order}}</td>
                            <td>
                                <a class="btn btn-outline btn-circle btn-sm purple"
                                   href="{{route('getContentById',['contentId'=>$content->id])}}">
                                ...
                                <form id="form_delete" class="form-delete-c" method="post"
                                      ...
                                      action="{{route('deleteContentById',['sectorId'=>$content->id])}}">
                                    {{ csrf_field() }}
                                </form>
                            </td>
                        </tr>
