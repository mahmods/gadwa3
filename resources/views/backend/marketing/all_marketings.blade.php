...
@section('breadcrumb')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="#">{{trans('marketing.marketings')}}</a>
            </li>
...
<div class="row">
    <div class="col-md-12">
        <div class="btn-group">
            <a class="btn green" href="{{route('getAddMarketing')}}"><i class="fa fa-level-down"></i> {{trans('marketing.add_marketing')}}
            </a>
        </div>
        <br clear="all">
        <br clear="all">
        ...
        <div class="portlet box gadwa">
            <div class="portlet-title">
                <div class="caption">
                    <i class="fa fa-globe"></i>{{trans('marketing.all_marketings')}}</div>
                <div class="tools"></div>
            </div>
            <div class="portlet-body">
                <table class="table table-striped table-bordered table-hover" id="sample_2">
                    ...
                    <tr>
                        <th>#</th>
                        <th>{{trans('marketing.title')}}</th>
                        <th>{{trans('marketing.description')}}</th>
                    ...
                    @foreach($marketings as $i=>$marketing)
                        <tr class="odd gradeX">
                            ...
                            <td>{{$marketing->study->title}}</td>
                            <td>{{$marketing->description}}</td>
                            <td>
                                <a class="btn btn-outline btn-circle btn-sm purple"
                                   href="{{route('getMarketingById',['marketingId'=>$marketing->id])}}">
                                ...
                                <form id="form_delete" class="form-delete-c" method="post"
                                      ...
                                      action="{{route('deleteMarketingById',['marketingId'=>$marketing->id])}}">
                                    {{ csrf_field() }}
                                </form>
                            </td>
                        </tr>
