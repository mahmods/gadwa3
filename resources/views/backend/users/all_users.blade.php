...
@section('breadcrumb')
    <div class="page-bar">
        <ul class="page-breadcrumb">
            ...
            <li>
                <a href="#">{{trans('users.header_bar')}}</a>
            </li>
...
<div class="row">
    <div class="col-md-12">
        ...
        <div class="btn-group">
            <a class="btn green" href="{{route('getAddUser')}}"><i class="fa fa-user-plus"></i> {{trans('users.addNew')}}
            </a>
        </div>
        <br clear="all">
        <br clear="all">
                ...
                <table class="table table-striped table-bordered table-hover" id="sample_2">
                    ...
                    @foreach($users as $i=>$user)
                        <tr class="odd gradeX">
                            ...
                            <td>{{$user->name}}</td>
                            <td>{{$user->email}}</td>
                            <td>{!! trans('backend.'. \App\Repositories\RolesRepository::getAllRolesToUsers()->where('role_id',$user->role_id)->first()['role_name']) !!}</td>
                            <td>
                                <a class="btn btn-outline btn-circle btn-sm purple"
                                   href="{{route('getUserById',['id'=>$user->id])}}">
